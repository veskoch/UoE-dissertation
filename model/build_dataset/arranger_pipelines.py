...
class TransposerToC(NoteSequencePipeline):
    ...
    def __init__(self, name, min_valid_pitch, max_valid_pitch):
        ...
        self.min_valid_pitch = min_valid_pitch
        self.max_valid_pitch = max_valid_pitch
        print('INFO: Transposing all to C.')

    def transform(self, sequence):
        key = sequence.key_signatures[0].key
        transposed = self._transpose(sequence, -key)
        if transposed is not None:
            # self._pitch_freq_counter(transposed)
            return [transposed]
    ...
    def _pitch_freq_counter(self, sequence):
        ...
        stats = dict([(stat_name, statistics.Counter(stat_name)) for stat_name in
                list(map(str,list(range(self.min_valid_pitch,self.max_valid_pitch)))) ])
