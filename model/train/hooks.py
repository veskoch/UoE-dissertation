    ...
    with self._graph.as_default():
      (self._scaffold, self._update_op, self._eval_dict,
       self._all_hooks) = self._estimator._evaluate_build_graph(
           self._input_fn, self._hooks, checkpoint_path=self._checkpoint_path)
...
class EvaluationAtStepHook(tf.train.SessionRunHook):
  ...
  def _evaluate(self):
    if self._checkpoint_path is not None and os.path.isdir(self._checkpoint_path):
      checkpoint_path = tf.train.latest_checkpoint(self._checkpoint_path)

      self._estimator.evaluate(
        self._eval_spec.input_fn, hooks=self._eval_spec.hooks, checkpoint_path=checkpoint_path)